...
export default function HelpPage() {
    ...
    useEffect(() => {
        ...
        const results: (typeof helpCategories) = [];
        helpCategories.forEach(category => {
            const titleMatches = category.title.toLowerCase().includes(lowercasedQuery);
            const matchingLinks = category.links.filter(link =>
                link.toLowerCase().includes(lowercasedQuery)
            );
                ...
                <div className="container mx-auto px-4 relative z-10">
                    ...
                    <div className="relative max-w-2xl mx-auto mt-8">
                        <Input 
                            ...
                            className="h-12 pl-12 text-lg text-black"
                            value={searchQuery}
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
                    {noResults ? (
                        <div className="col-span-full text-center py-16">
                            <h2 className="font-headline text-2xl font-bold">No results found for "{searchQuery}"</h2>
                            <p className="text-muted-foreground mt-2">Please try searching for a different topic.</p>
                        </div>
                    ) : (
